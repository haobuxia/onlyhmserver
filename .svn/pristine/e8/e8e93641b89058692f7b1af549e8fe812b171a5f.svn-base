...
public class ConnectionPool {
    ...
    private String configFileParentDirName;

    /**
     * 默认构造方法
     */
    public ConnectionPool(long minPoolSize, long maxPoolSize, long waitTimes, String configFileParentDirName) {
        this.configFileParentDirName = configFileParentDirName;
            ...
            if (nowPoolSize < maxPoolSize) {
                ...
                try {
                    ...
                    LOGGER.error("[获取空闲连接(checkout)-error][" + logId
                            + "][error:获取连接超时:{}]", e);
                    throw new FdfsException("获取空闲连接超时", e);
                }
            }
            if (trackerServer == null) {
                LOGGER.error("[获取空闲连接(checkout)-error][" + logId
                        + "][error:获取连接超时（" + waitTimes + "s）]");
                throw new FdfsException("获取空闲连接超时");
            }
...
//                + "] ");
    ...
    private void initClientGlobal() throws Exception {
        String configFileName = configFileParentDirName + "/fastdfs-client.properties";
        LOGGER.info("FastDfsClientInit file = " + configFileName);
        URL url = FastDfsClient.class.getClassLoader().getResource(configFileName);
        if (url != null) {
            String file = url.getFile();
            LOGGER.info("FastDfsClientInit file = " + file);
            try {
                ClientGlobal.initByProperties(file);
            } catch (Exception e) {
